...
'''
 ...
 to Huawei OBS using the OBS SDK for Python.
'''
...
obsClient.putFile(sourceBucketName, sourceObjectKey, createSampleFile(sampleFilePath))
...
for i in range(partCount):
    ...
    rangeEnd = objectSize if (i + 1 == partCount) else rangeStart + partSize

    p = proc(target=doCopyPart, args=(lock, partETags, bucketName, objectKey, i+1, uploadId, sourceBucketName + '/' + sourceObjectKey, str(rangeStart) + '-' + str(rangeEnd)))
    p.daemon = True
    processes.append(p)
